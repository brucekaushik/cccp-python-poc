...
class AsciiToJsonIr(IrContext):
    ...
    def encode(self) -> None:
        ...
        if not self.ir["segments"]:
            ...
            self.encode_excluded_segments()

    def encode_file(self) -> None:
        self.processed_segments = []

        with open(self.input_filepath, "r", encoding="ascii") as fp:
            self.encode_stream(fp)

        self.ir["segments"] = self.processed_segments

    def encode_excluded_segments(self) -> None:
        segments = []
        ...
        for segment in self.ir["segments"]:
            ...
            self.header_code_curr = None
            self.header_code_prev = None
